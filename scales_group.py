...
def learn(G=None, Phi=None, base_image_dim=32*32, patch_dim=9*9, scales=2, overcomplete=1, iterations=4000, inf_iterations=150, batch=100, 
    ...
    groups = parent_groups(base_image_side, scales)
    ...
    for t in range(iterations+1):
      ...
      A = inference(I, G, Phi, base_image_dim, lambdav, algorithm='fista-gpu', max_iterations=inf_iterations, max_eig=max_eig, groups=groups)
...
def inference(I, G, Phi, base_image_dim, lambdav, algorithm='fista', max_iterations=100, max_eig=None, groups=None):
  scales = len(Phi)
  ...
  elif algorithm == 'fista-l1l2':
    M = sps.hstack([G[s].dot(Phi[s]) for s in range(scales)]).tocsr()
    L = fista.fista(I, M, lambdav, max_iterations=max_iterations, display=False, problem='l1l2', groups=groups)
    A = np.vsplit(L, np.cumsum([base_image_dim/4**(scales-s-1) for s in range(scales)]))
    A.pop(-1)

  elif algorithm == 'fista-gpu':
    M = sps.hstack([G[s].dot(Phi[s]) for s in range(scales)]).tocsr()
    L = gpu.fista(I, M, lambdav, max_iterations=max_iterations, display=True, verbose=True, L=None)
    A = np.vsplit(L, np.cumsum([base_image_dim/4**(scales-s-1) for s in range(scales)]))
    A.pop(-1)
...
def parent_groups(base_image_side, scales):
  """ Returns indices of groups. Assumes simple parent-child group structure. (groups x indices) """

  def non_overlapping_blocks(image_side, offset, block_side):
    """ Returns matrix where each row is the reshaped (block_side x block_side) block """
    a = np.arange(offset, image_side**2 + offset).reshape(image_side, image_side)
    h, w = a.shape
    shape = (h/block_side, w/block_side, block_side, block_side)

    strides = a.itemsize*np.array([w*block_side, block_side, w, 1])
    return np.lib.stride_tricks.as_strided(a, shape=shape, strides=strides).reshape((image_side/block_side)**2, block_side**2)

  top_neurons = (base_image_side**2)/4**(scales-1) # neurons on top layer
  num_groups = top_neurons
  num_indices = np.sum(4**np.arange(scales-1)) # receptive fields in a group

  g = np.empty((top_neurons,0)).astype(int)
  for s in range(scales):
    image_side = base_image_side/2**(scales-s-1)
    offset = np.sum(4**np.arange(s)*top_neurons)

    g = np.hstack((g, non_overlapping_blocks(image_side, offset, 2**s)))
